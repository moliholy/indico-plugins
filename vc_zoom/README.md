...
- Allow setting the webhook secret token in `indico.conf` (`PLUGIN_VC_ZOOM_WEBHOOK_TOKEN`); when set,
  it takes precedence over the plugin setting, which is then hidden from the settings page
...
in `indico.conf`, in which case the plugin setting is hidden:
 ...
 * **Webhook Secret Token** (optional) - the token which Zoom requests will authenticate with (get it from Zoom Marketplace);
   not shown when `PLUGIN_VC_ZOOM_WEBHOOK_TOKEN` is set in `indico.conf`
