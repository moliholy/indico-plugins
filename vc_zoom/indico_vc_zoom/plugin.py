...
class ZoomPlugin(VCPluginMixin, IndicoPlugin):
    ...
    def _registration_zoom_emails(self, registration):
        emails = {registration.email.lower()}
        if registration.user:
            emails |= {e.lower() for e in registration.user.all_emails}
        return emails
    ...
    def get_registration_sync_counts(self, vc_room):
        ...
        registrations = {}
        for reg in self._collect_complete_registrations(vc_room):
            registrations.setdefault(reg.email.lower(), reg)
        total = len(registrations)
        zoom_emails = self._list_zoom_registrant_emails(vc_room)
        if zoom_emails is None:
            registered = None
        else:
            registered = sum(1 for reg in registrations.values()
                             if self._registration_zoom_emails(reg) & zoom_emails)
